...
export default function LabFollowUp() {
    ...
    const {data: LabData, loading, error, refetch: refetchLabData} = useQuery(LOAD_LAB_FOLLOW_UP, {fetchPolicy: "network-only"})
    const {data: clinicsData } = useQuery(LOAD_CLINICS,{ variables: {branch_id: Number(branchId)}, fetchPolicy: 'network-only', skip: !branchId})
    const {data: ordersData } = useQuery(LOAD_ORDERS,{ fetchPolicy: 'network-only'})
...
useEffect(() => {
    ...
    ...(statusData?.lab_follow_up_statusCollection?.edges?.map(({ node }) => ({
        label: node.status,
        value: node.status
    })) || [])
];
