...
export default function LabFollowUp() {
    ...
    const readyToSendOrders = branchOrderEdges.filter(({ node }) => {
        const reminderCall = node.reminder_callCollection?.edges?.[0]?.node;
        const beforeLabStatus = reminderCall?.before_lab_status;
        const beforeLabReason = reminderCall?.before_lab_reason;
        return beforeLabStatus === 'answer' && beforeLabReason === 'readytosend';
    });
