...
export default function PaymentMonitoring() {
  ...
  // 🔹 GraphQL
  const { data, loading, error } = useQuery(GET_PAYMENT_MONITORING);
  ...
  const tableData =
    data?.orderCollection?.edges?.map((item, index) => {
      ...
      const paid = advance + deliveryPayments;

      // Outstanding balance
      const pending = Math.max(0, revenue - paid);

      // Collection percentage
      const rate = revenue > 0 ? ((paid / revenue) * 100).toFixed(1) : "0.0";
  ...
  const filteredData =
    selectedBranch === "all"
      ? tableData
      : tableData.filter((item) => item.branch === selectedBranch);
  ...
  const branchOptions = [
    ...
    ...[...new Set(tableData.map((item) => item.branch).filter(Boolean))].map(
      (branch) => ({
        value: branch,
        label: branch,
      }),
  ...
  const columns = [
    {
      title: "Order ID",
      dataIndex: "orderId",
      render: (text) => <Tag color="blue">ORD-{text}</Tag>,
    },

    {
      title: "Customer ID",
      dataIndex: "customerId",
      render: (text) => <Tag color="purple">CUS-{text}</Tag>,
    },
...

    {
      ...
      dataIndex: "rate",
      render: (text) => (
        <Tag color={text === "100.0%" ? "green" : "orange"}>{text}</Tag>
      ),
    },
  ];
